 ...
 * kynet's network is divided into several smaller networks, in each sub-network is a Skynet agent tasked with transferring information by moving from node to node along links and accessing gateways leading to other sub-networks.
 *
 ...
 * There are 4 nodes, 4 links and 1 gateway. The next 4 lines describe the links.
 * The last integer is the index of the exit node.
 *
 * 4 4 1
 * 0 1
 * 0 2
 ...
 * Then, within an infinite loop, read the data from the standard input related to the current state of the Skynet agent and provide to the standard output the next instruction.
 *
 * Initialization input
 ...
 * A single line comprised of two integers C1 and C2 separated by a space.
